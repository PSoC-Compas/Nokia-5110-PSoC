...
/* PRT0DR = `$INSTANCE_NAME`__RST_PIN; */
/* Enable SPI */
...
***************************************************************************/
#ifdef DRAW_OVER_BACKGROUND
void `$INSTANCE_NAME`_Chr ( char ch, draw_type dtype, front_type ft ){
    int i;
    
    switch(ft){
    case 0: // Small Front
        if (dtype == DRAW_OR)  
            for(i=0; i<3; i++, Background_cursor++) `$INSTANCE_NAME`_SendData(SmallFront[ch-32][i] | Background[Background_cursor]);
...
#else //DRAW_OVER_BACKGROUND
void `$INSTANCE_NAME`_Chr ( char ch , front_type ft ){
    int i;
    switch(ft){
    case 0: // Small Front
...
***************************************************************************/
#ifdef DRAW_OVER_BACKGROUND
void `$INSTANCE_NAME`_Str ( char *dataPtr, draw_type dtype, front_type ft ){
    int i;
    while ( *dataPtr ){
        char ch = *dataPtr++; 
        switch(ft){
...
#else //DRAW_OVER_BACKGROUND
void `$INSTANCE_NAME`_Str ( char *dataPtr , front_type ft  ){
    int i;
    while ( *dataPtr ){
